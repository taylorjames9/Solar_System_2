...
[CustomPropertyDrawer(typeof(ForceField.DistanceModifier))]
public class DistanceModifierDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (FFEditorToolKit.DrawHeader(label.text))
        {
            ...
            if (modifierType.enumValueIndex == (int)ForceField.DistanceModifier.EDistanceModifier.AnimationCurve)
            {
            ...
            else if (modifierType.enumValueIndex != (int)ForceField.DistanceModifier.EDistanceModifier.Constant)
            {
                EditorGUILayout.PropertyField(property.FindPropertyRelative("boundAtZero"));
